...
const { getAiEnabled, pushEvent } = require("../services/web");
    ...
    while (retries > 0) {
        try {
            ...
            if (typeof pushEvent === "function") {
                pushEvent("MSG_OUT", { ct: Object.keys(content)[0], to: jid });
            }
            return res;
        } catch (err) {
...
async function handleMessage(sock, msg) {
    try {
        ...
        // Subscribe to presence so dashboard receives typing/composing events
        try { await sock.presenceSubscribe(sender); } catch (_) { /* ignore */ }

        // ?? Dashboard structured log (parsed by dashboard.py) ??
        const _dashMediaTypes = ['imageMessage','videoMessage','audioMessage','documentMessage','gifMessage'];
        const _dashType = _dashMediaTypes.includes(msgType)
            ? msgType.replace('Message','').toUpperCase()
            : 'TEXT';
        console.log(`?? [INCOMING] JID:${sender} NAME:${pushName} TYPE:${_dashType} MSG:${text || '[No Text]'}`);
        if (typeof pushEvent === "function") {
            pushEvent("MSG_IN", { jid: sender, name: pushName, mtype: _dashType, content: text || '[No Text]' });
        }
        ...
        if (needsScrape) {
            try {
                ...
                console.error("?? [SCRAPER] Error gathering first-contact data:", err.message);
            }
        }
        ...
        if (mediaTypes.includes(currentMsgType)) {
            ...
            try {
                ...
                const buffer = await downloadMediaMessage(msg, 'buffer', {}).catch(e => {
                    console.warn(`?? [SYSTEM] Media Download Failed: ${e.message}`);
                    return null;
                });
        ...
        if (lower === "reset" || lower === "clear chat") {
            ...
            await safeSendMessage(sock, sender, { text: "?? [RESET] Aapki history, stop status aur cached profile data clear kar diya hai yaar. Ab fresh start aur re-scraping hogi! ??" }, { quoted: msg });
            return;
        }
        ...
        if (lower === "status") {
            ...
            const list = entries.map(([jid, d]) => `• ${jid.split('@')[0]}: ${d.status === 'available' ? '?? online' : d.status === 'composing' ? '?? typing...' : '? offline'}`).join('\n');
        ...
        if (lower.startsWith("fs ")) {
            ...
            if (cmdLower === "delete") {
                ...
                await safeSendMessage(sock, sender, { text: `??? File *${safeName}* deleted.` }, { quoted: msg });
                return;
            }
        }
        ...
        // NO TYPING STATUS (Don't show typing dots to users)
        ...
        if (isImage || isVideo || isGif) {
            ...
            if (isImage) {
                ...
                console.log(`??? [SYSTEM v17.0-MESSENGER] Extracting ${typeLabel} Thumbnail for Vision API...`);
                ...
                if (thumbnail) {
                    ...
                    console.log(`?? [SYSTEM] No thumbnail found for ${typeLabel}, attempting full download fallback...`);
                    mediaBuffer = await downloadMediaMessage(msg, 'buffer', {}).catch(() => null);
                }
            } else {
        ...
        // ?? AI is disabled by default — admin must enable via dashboard ??
        if (!getAiEnabled()) {
            console.log(`?? [AI MUTED] Message from ${pushName} not processed — AI disabled by admin.`);
            return; // Silent — no reply
        }

        // --- AI TRIGGER LOGIC (Strictly @mazhar or mazhar Keyword only) ---
        const botJid = sock.user?.id ? (sock.user.id.split(':')[0] + '@s.whatsapp.net') : null;
        const mentionedJids = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [];
        const isOwnerTagged = mentionedJids.includes(OWNER_JID);
        const isBotTagged = botJid && mentionedJids.includes(botJid);

        // Match when user types "@mazhar" or contains "mazhar" (case-insensitive)
        const hasMazharKeyword = lower.includes("@mazhar") || lower.includes("mazhar") || isOwnerTagged || isBotTagged;

        if (!hasMazharKeyword) {
            console.log(`? [AI BYPASS] Message from ${pushName} did not contain '@mazhar' or 'mazhar'.`);
            return;
        }

        // Update the last AI trigger timestamp
        if (!userStats[sender]) {
            userStats[sender] = { messages: 0, firstSeen: new Date() };
        }
        userStats[sender].lastAiTime = Date.now();

        // Clean up prompt text of `@mazhar` or `mazhar` prefix
        if (prompt) {
            const quotedMatch = prompt.match(/^\[USER_REPLY_TO:\s*".*?"\]\s*/);
            if (quotedMatch) {
                const prefix = quotedMatch[0];
                let actualPrompt = prompt.substring(prefix.length);
                actualPrompt = actualPrompt.replace(/@mazhar/gi, "").trim();
                if (actualPrompt.toLowerCase().startsWith("mazhar")) {
                    actualPrompt = actualPrompt.substring(6).trim();
                }
                actualPrompt = actualPrompt.replace(/^[:\-\s,]+/, "").trim();
                prompt = prefix + actualPrompt;
            } else {
                prompt = prompt.replace(/@mazhar/gi, "").trim();
                if (prompt.toLowerCase().startsWith("mazhar")) {
        ...
        // Send composing presence only (do not send random emojis as messages to WhatsApp)
        try {
            await sock.sendPresenceUpdate('composing', sender);
            console.log(`?? [PRESENCE] Set composing for ${sender}`);
        } catch (_) { /* ignore */ }

        let reply = await mazharAiReply(prompt, sender, pushName, mediaBuffer, mediaType);
        if (!reply) {
            // Set presence back to available even if no reply
            try { await sock.sendPresenceUpdate('available', sender); } catch (_) { /* ignore */ }
            return; // Silent or break mode active
        }
        ...
        cleanReply = cleanReply
            .replace(/Bhai, tumne to Mazhar Aslam ?? pata lagaya!/gi, "")
            .replace(/Bhai, tumne to Mazhar Aslam ?? pucha hai!/gi, "")
            .replace(/WhatsApp ?? rules/gi, "")
        ...
        console.log(`?? [AI REPLY TO:${sender}] ${cleanReply.substring(0, 120)}...`);
        if (typeof pushEvent === "function") {
            pushEvent("AI_REPLY", { jid: sender, snippet: cleanReply });
        }
        ...
        if (cleanReply.includes("[GLOBAL_MEMORY_RESET]")) {
            conversationMemory.delete(sender);
            console.log(`?? [SYSTEM] Global Memory Reset triggered for ${sender}`);
                ...
                if (researchResult.images.length > 0) {
                    for (const imgUrl of researchResult.images) {
                        try {
                            ...
                            console.warn("?? [RESEARCH] Skipping broken image URL:", imgUrl);
                        }
                    }
                }
        ...
        try {
            ...
            console.log(`?? [PRESENCE] Set available for ${sender}`);
        } catch (_) { /* ignore */ }
...
// Presence handler — logs for dashboard typing UI (never sends to WhatsApp)
function handlePresence(update) {
    ...
    for (const [participant, data] of Object.entries(presences)) {
        if (!data) continue;
        const status = data.lastKnownPresence || "offline";
        const jid = participant.includes("@") ? participant : id;
        const prev = userPresences[jid]?.status;
        if (!userPresences[jid]) userPresences[jid] = { status: "offline" };
        userPresences[jid].status = status;
        if (prev !== status) {
            console.log(`?? [PRESENCE] JID:${jid} STATUS:${status}`);
            if (typeof pushEvent === "function") {
                pushEvent("PRESENCE", { jid, status });
            }
        }
    }
}

module.exports = { handleMessage, handlePresence, userPresences };
