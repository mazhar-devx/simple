...
function washHistory(memory) {
    return memory.map(m => {
        if (m.role === "assistant") {
            ...
            content = content.replace(/Bhai, tumne to Mazhar Aslam ?? pata lagaya!/gi, "");
            content = content.replace(/Bhai, tumne to Mazhar Aslam ?? pucha hai!/gi, "");
            content = content.replace(/WhatsApp ?? rules/gi, "");
...
async function transcribeVoice(buffer) {
    ...
    if (!apiKey) throw new Error("Groq API key missing — set GROQ_API_KEY in .env");
    ...
    try {
        ...
        while (retries > 0) {
            try {
                ...
                console.warn(`?? [VOICE] Connection error (${err.code}). Retrying...`);
                retries--;
                if (retries === 0) {
...
async function mazharAiReply(userMessage, senderJid, userName = "User", mediaBuffer = null, mediaType = null) {
    ...
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey) return "?? Groq API key is missing in environment. Set GROQ_API_KEY in .env";
    ...
    try {
        ...
        while (retries > 0) {
            try {
                ...
                console.warn(`?? [AI] Network drop (${err.code}). Rebooting connection... (${retries - 1} left)`);
                retries--;
                if (retries === 0) {
        ...
        if (!res.ok) {
            let errorData = {};
            try { errorData = await res.json().catch(() => ({})); } catch (e) { }
            ...
            if (isDecommissioned && model.includes("vision")) {
                console.warn(`?? [AI] Vision model failed. Trying Llama 4 Scout Fallback...`);
                res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                ...
                if (isHardQuota) {
                    console.warn(`?? [AI] Hard Quota Exhausted on ${model}. Trying Vision Fallbacks or Text shield...`);
                } else {
                    console.warn(`?? [AI] Rate Limit Hit (429) on ${model}. Falling back to other models...`);
                }
                ...
                for (const fallbackModel of fallbackModels) {
                    ...
                    if (fallbackRes.ok) {
                        ...
                        try { const fallbackErr = await fallbackRes.json().catch(() => ({}));
                ...
                if (!fallbackSuccess && hasImage) {
                    console.warn(`?? [AI] Vision fallbacks failed. Stripping images and falling back to text models...`);
